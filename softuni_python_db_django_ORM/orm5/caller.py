...
def find_authors_nationalities():
    ...
    result = [f"{author.first_name} {author.last_name} is {author.nationality}" for author in filtered_authors]
    return '\n'.join(result)


def order_books_by_year():
    ordered_books = Book.objects.order_by('publication_year', 'title')
    result = [f"{book.publication_year} year: {book.title} by {book.author}" for book in ordered_books]
    return '\n'.join(result)


def delete_review_by_id(id_ref: int) -> str:
    review = Review.objects.get(id=id_ref)
    review.delete()
...
def filter_authors_by_birth_year(start_year: int, end_year: int) -> str:
    selected_authors = Author.objects.filter(birth_date__year__range=(start_year, end_year)).order_by('-birth_date')
    result = [f"{a.birth_date}: {str(a)}" for a in selected_authors]
    return '\n'.join(result)
