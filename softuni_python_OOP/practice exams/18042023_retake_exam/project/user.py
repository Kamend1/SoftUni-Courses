class User:
    INCREMENTAL_INCREASE = 0.5
    INCREMENTAL_DECREASE = 2.0
    MAX_RATING = 10
    MIN_RATING = 0
    ...
    def increase_rating(self):
        self.rating += self.INCREMENTAL_INCREASE

        if self.rating > self.MAX_RATING:
            self.rating = self.MAX_RATING

    def decrease_rating(self):
        if self.rating - self.INCREMENTAL_DECREASE <= 0:
            self.rating = 0
            self.is_blocked = True
        else:
            self.rating -= self.INCREMENTAL_DECREASE
